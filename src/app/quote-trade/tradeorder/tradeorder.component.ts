...
export class TradeorderComponent implements OnInit {
  ...
  onSubmitHandler(e) {
    ...
    this.http.post(this.utilsService.getFullUrl('node', 'setorder'), orderData, this.httpOptions).subscribe({
      next: order => {
        this.fetchOrders();
      },
  ...
  fetchOrders() {
    this.http.get<Orders>(this.utilsService.getFullUrl('node', 'getorder'), this.httpOptions)
      .pipe(pluck('orders'))
      .subscribe(orders => {
